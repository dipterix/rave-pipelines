    ...
    input_path_ct_in_t1 = targets::tar_target_raw("path_ct_in_t1", 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("fslut", 
                  e, quote({
                    fslut_path <- system.file("palettes", "datacube2", 
                      "FreeSurferColorLUT.json", package = "threeBrain")
                    cmap <- threeBrain::load_colormap(fslut_path)
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("ct_candidates", 
                  e, quote({
                    fs_path <- subject$freesurfer_path
                    if (length(fs_path) != 1 || is.na(fs_path) || 
                      !dir.exists(fs_path)) {
                    ...
                    f1 <- list.files(file.path(fs_path, "coregistration"), 
                      ...
                      ignore.case = TRUE, include.dirs = FALSE, 
                      full.names = FALSE, all.files = FALSE)
                    f2 <- list.files(file.path(fs_path, "..", 
                      "coregistration"), pattern = "nii(?:\\.gz)?$", 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("plan_list", 
                  e, quote({
                    require(dipsaus)
                    count <- dipsaus::fastmap2()
                    count$n <- 0
                    ...
                    plan_table <- lapply(localization_plan, function(item) {
                      ...
                      re <- data.frame(Electrode = count$n + 
                        seq_len(ne), Label = sprintf("%s%d", 
                        label, count$labels[[label]] + seq_len(ne)), 
                        LabelPrefix = label, Dimension = paste(dim, 
                          collapse = "x"), LocationType = type)
                      count$n <- count$n + ne
                      count$labels[[label]] <- count$labels[[label]] + 
                        ne
                    ...
                    if (length(plan_table)) {
                      ...
                      try({
                        if (length(files)) {
                          files <- files[[1]]
                          electrode_table <- raveio::safe_read_csv(files)
                          tname1 <- c("Electrode", "Coord_x", 
                            "Coord_y", "Coord_z")
                          tname2 <- c("Electrode", "Coord_x", 
                            "Coord_y", "Coord_z", "Radius", "SurfaceType", 
                            "Hemisphere", "FSIndex", "FSLabel", 
                            "MNI305_x", "MNI305_y", "MNI305_z")
                          if (all(tname1 %in% names(electrode_table))) {
                            tname2 <- tname2[tname2 %in% names(electrode_table)]
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("brain", 
                  e, quote({
                    brain <- threeBrain::freesurfer_brain2(fs_subject_folder = subject$freesurfer_path, 
                      subject_name = subject$subject_code)
                  }))
        ...
        iteration = "list"), Loading_brain_and_CT_if_exists = targets::tar_target_raw(name = "ct_in_t1", 
        command = quote({
            tryCatch({
                {
                  force(subject)
                  ...
                  if (length(path_ct_in_t1)) {
                    if (startsWith(path_ct_in_t1, "{")) {
                      ...
                      ct_path <- normalizePath(path_ct_in_t1, 
                        mustWork = TRUE)
                    }
                    has_ct <- TRUE
                  }
                ...
                asNamespace("raveio")$resolve_pipeline_error("ct_in_t1", 
                  e, quote({
                    force(subject)
                    has_ct <- FALSE
                    ct_path <- character(0L)
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("ct_exists", 
                  e, quote({
                    ct_exists <- isTRUE(!is.null(ct_in_t1) && 
                      is.list(ct_in_t1))
                  }))
            })
        }), format = asNamespace("raveio")$target_format_dynamic(NULL), 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("viewer", 
                  e, quote({
                    if (!is.null(ct_in_t1) && is.list(ct_in_t1)) {
                      viewer <- brain$localize(coregistered_ct = ct_in_t1)
                    } else {
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("localization_result_initial", 
                  e, quote({
                    require(dipsaus)
                    re <- lapply(localization_list, function(item) {
                      item$FSIndex %?<-% 0
        ...
        deps = c("localization_list", "brain", "subject"), cue = targets::tar_cue("thorough"), 
        pattern = NULL, iteration = "list"), calculate_std141_mappings = targets::tar_target_raw(name = "localization_result_final", 
        command = quote({
            tryCatch({
                {
                ...
                asNamespace("raveio")$resolve_pipeline_error("localization_result_final", 
                  e, quote({
                    src <- file.path(subject$meta_path, "electrodes_unsaved.csv")
                    if (file.exists(src)) {
                      re <- utils::read.csv(file.path(subject$meta_path, 
