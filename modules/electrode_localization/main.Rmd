...
tryCatch({
  ...
  if(!is.na(fs_path) && length(fs_path) == 1 && file.exists(fs_path)) {
    ...
    tmpdir <- file.path(subject$freesurfer_path, "tmp")
    ...
    if(!file.exists(lh_envelope_path) || !file.exists(rh_envelope_path)) {
      ...
      progress <- dipsaus::progress2("Compute pial envelope", max = 4, 
                                     shiny_auto_close = TRUE)
      
      # generate volume filled
      raveio::dir_create2(tmpdir)
      progress$inc("Filling lh.pial...")
      if( !file.exists(lh_filled_path) ) {
        threeBrain::fill_surface(
          surface = lh_pial_path, save_as = lh_filled_path, 
          resolution = 256L, delta = 3, )
      }
      
      progress$inc("Filling rh.pial...")
      if( !file.exists(rh_filled_path) ) {
        threeBrain::fill_surface(
          surface = rh_pial_path, save_as = rh_filled_path, 
          resolution = 256L, delta = 3)
      }
      ...
      if( file.exists(lh_filled_path) ) {
        threeBrain::generate_smooth_envelope(
          filled_volume_path = lh_filled_path, 
      ...
      if( file.exists(rh_filled_path) ) {
        threeBrain::generate_smooth_envelope(
          filled_volume_path = rh_filled_path, 
