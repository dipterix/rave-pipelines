...
```{rave check_prerequisite, language = "R", export = "notch_filtered_electrodes"}
...
```{rave get_raw_sample_rates, language = "R", export = "sample_rates"}
...
```{rave check_kernel, language = "R", export = "kernels"}
...
```{rave run_wavelet, language = "R", export = "wavelet_params"}
...
lapply(unique(srates), function(srate) {
  lapply(blocks, function(block){
    ...
    generate_kernel(freqs = kernels$freqs, srate = srate, wave_num = kernels$cycles, data_length = data_length)
  })
})
...
preproc <- raveio::RAVEPreprocessSettings$new(subject = subject$subject_id, read_only = FALSE)
