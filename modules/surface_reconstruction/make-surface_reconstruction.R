    ...
    input_skip_coregistration = targets::tar_target_raw("skip_coregistration", 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("cmd_tools", 
                  e, quote({
                    default_dcm2niix_path <- raveio:::cmd_dcm2niix(error_on_missing = FALSE)
                    dcm2niix <- tryCatch({
                      dcm2niix <- raveio::normalize_commandline_path(path = dcm2niix_path, 
                    ...
                    freesurfer <- tryCatch({
                      ...
                      if (!path_is_valid(freesurfer, dir_ok = TRUE)) {
                        freesurfer <- NULL
                      } else if (!identical(default_fs_path, 
                        freesurfer)) {
                        raveio::raveio_setopt("freesurfer_path", 
                          freesurfer)
                      }
                    ...
                    afni <- tryCatch({
                      ...
                      if (path_is_valid(afni, dir_ok = TRUE)) {
                        if (!identical(default_afni_path, afni)) {
                          raveio::raveio_setopt("afni_path", 
                            afni)
                        }
                      } else {
                        afni
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("check_result", 
                  e, quote({
                    msgs <- character(0L)
                    warns <- character(0L)
                    fs_path <- subject$freesurfer_path
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("import_T1", 
                  e, quote({
                    import_T1 <- tryCatch({
                      raveio::cmd_run_dcm2niix(subject = subject, 
                        src_path = check_result$path_mri, type = "MRI", 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("import_CT", 
                  e, quote({
                    import_CT <- tryCatch({
                      raveio::cmd_run_dcm2niix(subject = subject, 
                        src_path = check_result$path_ct, type = "CT", 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("fs_recon", 
                  e, quote({
                    fs_recon <- tryCatch({
                      mri_path <- params$nii_t1
                      mri_root <- file.path(check_result$path_temp, 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("coreg_flirt", 
                  e, quote({
                    coreg_flirt <- tryCatch({
                      mri_path <- params$nii_t1
                      mri_root <- file.path(check_result$path_temp, 
        ...
        command = quote({
            tryCatch({
                ...
                asNamespace("raveio")$resolve_pipeline_error("coreg_3dallineate", 
                  e, quote({
                    coreg_3dallineate <- tryCatch({
                      mri_path <- params$nii_t1
                      mri_root <- file.path(check_result$path_temp, 
